# Probe Driver Architecture Refactor Requirements
...
This document outlines the requirements for refactoring the ProbeDriver architecture from a monolithic design to a modular, maintainable structure. The refactor aims to eliminate signal duplication, improve testability, and establish industry-standard VHDL design patterns.

## 1. Project Overview

### 1.1 Background
The current ProbeDriver implementation has evolved into a monolithic structure with:
- Signal duplication between testbench and implementation
- Mixed responsibilities (state machine + interface logic)
- Redundant status registers
- Difficult testing and maintenance

### 1.2 Objectives
- **Primary**: Eliminate signal duplication and improve maintainability
- **Secondary**: Establish clean architecture patterns for future modules
- **Tertiary**: Improve testability and debugging capabilities

### 1.3 Success Criteria
- Zero signal duplication between components
- Each component has single, clear responsibility
- All components compile and pass unit tests
- New architecture follows industry best practices
...
ProbeDriver/
├── common/
│   └── probe_driver_pkg.vhd          # Shared types, constants, functions
├── core/
│   └── probe_driver_core.vhd         # Pure state machine logic
├── wrapper/
│   └── probe_driver_wrapper.vhd      # Top-level interface & control registers
└── testbench/
    ├── probe_driver_core_tb.vhd      # Unit tests for core logic
    └── probe_driver_wrapper_tb.vhd   # Integration tests
```

### 2.2 Signal Naming Conventions
- **Descriptive names**: Use full, descriptive signal names
- **Consistent prefixes**: `probe_` for probe-specific, `config_` for configuration
- **Direction suffixes**: `_input`, `_output` where helpful
- **Examples**:
  - `probe_trigger_input` (not `trig_in`)
  - `config_intensity_index` (not `Intensity_index`)
  - `probe_status_register` (not `status_register`)

### 2.3 Port Organization
Ports must be logically grouped in entity declarations:
```vhdl
...
### 2.4 Data Width Consistency
- **Status Register**: 16 bits (bits [4:0] for states, [15:5] reserved)
- **Duration**: 16 bits (reduced from 32 for consistency)
- **Cooldown**: 16 bits (reduced from 32 for consistency)
- **Intensity**: 8 bits (unchanged, sufficient for 0-100 range)

### 2.5 VHDL Standards
- **Language**: VHDL-2008
- **File Extension**: `.vhd` only
- **Naming**: PascalCase for entities/architectures, snake_case for signals/ports
- **Comments**: Inline comments for control structures
- **Constants**: Use constants instead of magic numbers

## 3. Implementation Plan

### 3.1 Iteration 1: Foundation (Week 1)
**Deliverables**:
- [ ] Create `probe_driver_pkg.vhd` with all shared types and constants
- [ ] Extract core state machine to `probe_driver_core.vhd`
- [ ] Update signal names to follow new conventions
- [ ] Basic compilation test for package and core

**Success Criteria**:
- Package compiles without errors
- Core state machine compiles after package
- All signal names follow new conventions
- No compilation errors

### 3.2 Iteration 2: Interface Layer (Week 2)
**Deliverables**:
- [ ] Create `probe_driver_wrapper.vhd` with clean interface
- [ ] Integrate clock divider logic
- [ ] Implement control register mapping
- [ ] Update status register to 16 bits
- [ ] Reduce duration/cooldown to 16 bits

**Success Criteria**:
- Wrapper compiles after core
- All data widths are consistent (16 bits)
- Control register mapping is clean and logical
- Clock divider integration works

### 3.3 Iteration 3: Testing & Documentation (Week 3)
**Deliverables**:
- [ ] Create unit testbench for core logic
- [ ] Create integration testbench for wrapper
- [ ] Verify all functionality works as expected
- [ ] Update documentation and README files
- [ ] Final integration testing

**Success Criteria**:
- All testbenches compile and run
- All tests pass
...
### 4.1 Unit Testing
- **Core Logic**: Test state machine transitions, timing, error handling
- **Package**: Test all utility functions and type conversions
- **Wrapper**: Test control register mapping and output assignments

### 4.2 Integration Testing
- **End-to-End**: Verify complete probe driver functionality
- **Interface**: Test control register behavior and output mapping
- **Clock Division**: Verify timing control works correctly

### 4.3 Testbench Standards
- **Naming**: `*_tb.vhd` for testbenches
- **Success Message**: Must print "ALL TESTS PASSED" or "SIMULATION DONE"
...
### 5.1 Code Quality
- **Readability**: Clear, descriptive signal and port names
- **Maintainability**: Single responsibility per component
- **Testability**: Each component can be tested independently
- **Documentation**: Inline comments and clear architecture

### 5.2 Performance Requirements
- **Latency**: No increase in probe trigger latency
- **Throughput**: Maintain existing pulse generation capability
- **Resource Usage**: No significant increase in FPGA resource usage

### 5.3 Compatibility Requirements
- **Interface**: Must work with existing Moku Cloud Compiler
- **Synthesis**: Must be synthesis-friendly for Vivado 2022.2
- **Simulation**: Must work with GHDL for development and testing

## 6. Risk Assessment

### 6.1 Technical Risks
- **Compilation Issues**: New architecture may introduce compilation errors
- **Timing Issues**: Clock divider integration may affect timing
- **Interface Changes**: New signal names may confuse existing users

### 6.2 Mitigation Strategies
- **Incremental Development**: Test each component individually
- **Backup Strategy**: Keep old implementation as reference
- **Comprehensive Testing**: Test all functionality before final integration

## 7. Acceptance Criteria

### 7.1 Functional Requirements
- [ ] All original probe driver functionality preserved
- [ ] No signal duplication between components
- [ ] Clean, logical port organization
- [ ] Consistent 16-bit data widths

### 7.2 Quality Requirements
- [ ] All components compile without errors
- [ ] All testbenches pass successfully
...
### 8.1 Extensibility
- **Status Register**: Reserved bits [15:5] available for future features
- **Configuration**: Easy to add new configuration parameters
- **Interface**: Modular design allows easy integration of new features

### 8.2 Reusability
- **Core Logic**: Can be reused in different wrapper contexts
- **Package**: Shared types and functions available for other modules
- **Patterns**: Architecture can serve as template for other modules

## 9. Appendices

### 9.1 Current Architecture Issues
- Signal duplication between testbench and implementation
- Mixed responsibilities in single file
- Inconsistent data widths
- Difficult testing and maintenance

### 9.2 Reference Documents
- Original ProbeDriver implementation
- Moku Cloud Compiler requirements
...
```markdown:moku-dev-vhdl/requirements/PRD-probe-driver-architecture-refactor.md
# Product Requirements Document (PRD)
